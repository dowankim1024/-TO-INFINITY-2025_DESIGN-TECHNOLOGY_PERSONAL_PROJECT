...
## 사용 기술 스택
- **Three.js**: WebGL 기반 3D 렌더링과 파티클 시스템 구현.
- **MediaPipe Selfie Segmentation**: 실시간 사람 실루엣 감지를 통한 인터랙티브 입력 처리.
- **WebGL**: GPU 가속 렌더링 파이프라인으로 Three.js 씬을 출력.
- **Simplex Noise (Perlin 변형)**: 배경 파티클 움직임에 자연스러운 노이즈 패턴 제공.
- **MediaDevices API (`getUserMedia`)**: 사용자의 웹캠 스트림을 획득하여 실시간 분석에 활용.
- **JavaScript (ES Module 구조)**: 모듈화된 애플리케이션 로직 구성 및 상태 관리.
- **HTML5 `<canvas>` & `<video>` 요소**: 3D 씬 렌더링과 마스크/디버그 캔버스 구성.
- **CSS3**: 전체 화면 UI 스타일링 및 전환 효과 정의.
